...
def is_banned(uid):
    ...
    if uid in muted:
        if muted[uid] == "forever": return True
        if datetime.now() < muted[uid]: return True
        else: del muted[uid]
    return False
...
def check_trial_limit(uid):
    ...
    if admins[uid]["level"] != 0: return True
    now = datetime.now()
    ...
    if admins[uid]["daily_answers"] >= TRIAL_DAILY_LIMIT: return False
    admins[uid]["daily_answers"] += 1
    return True

def user_menu():
    return ReplyKeyboardMarkup([[KeyboardButton("📩 Подать идею для ТГК")], [KeyboardButton("🎮 Вопрос по игре")], [KeyboardButton("📋 Мои заявки")]], resize_keyboard=True)

def admin_menu(uid):
    lvl = get_level(uid)
    buttons = []
    if lvl >= 0: buttons.append([KeyboardButton("🎮 Вопросы по игре")])
    if lvl == 3: buttons.append([KeyboardButton("📋 Идеи для ТГК")])
    if lvl >= 0: buttons.append([KeyboardButton("👑 Админ-панель")])
...
def parse_mute_time(text):
    ...
    if text == "навсегда": return "forever"
    ...
    if unit in ("минут","минута","минуты"): return timedelta(minutes=num)
    elif unit in ("час","часа","часов"): return timedelta(hours=num)
    elif unit in ("день","дня","дней"): return timedelta(days=num)
    elif unit in ("недел","неделя","недели","недель"): return timedelta(weeks=num)
    elif unit in ("месяц","месяца","месяцев"): return timedelta(days=num*30)
    return None

def format_mute_duration(duration):
    if duration == "forever": return "навсегда"
    total_seconds = duration.total_seconds()
    days, hours, minutes = int(total_seconds//86400), int((total_seconds%86400)//3600), int((total_seconds%3600)//60)
    parts = []
...
async def start(client, message):
    ...
    if is_banned(uid): await message.reply("Вы заблокированы или заглушены."); return
    if is_admin(uid): await message.reply("Приветствую, администратор.", reply_markup=admin_menu(uid))
    else: await message.reply("Добро пожаловать! Предложи идею для ТГК или задай вопрос по игре.", reply_markup=user_menu())
...
async def my_stuff(client, message):
    ...
    if not user_ideas and not user_questions: await message.reply("У тебя пока ничего нет.", reply_markup=back_button()); return
    text = ""
    if user_ideas:
...
async def active_ideas(client, message):
    if get_level(message.from_user.id) != 3: return
    pending = {k:v for k,v in apps.items() if v["status"] in ("pending","thinking")}
...
async def admin_questions(client, message):
    if get_level(message.from_user.id) < 0: return
...
async def my_history(client, message):
    ...
    if not my_q and not my_i: await message.reply("Ты пока ничего не обработал.", reply_markup=back_button()); return
    text = "📋 ТВОЯ ИСТОРИЯ:\n\n"
    if my_q:
        text += "🎮 Вопросы по игре:\n"
        for qid, data in my_q.items(): text += f"#{qid}: @{data['username']} — {data['text'][:30]}...\n"
    if my_i:
        text += "\n📩 Идеи для ТГК:\n"
        for iid, data in my_i.items(): text += f"#{iid}: @{data['username']} — {data['text'][:30]}...\n"
    await message.reply(text, reply_markup=back_button())

@app.on_message(filters.text & filters.regex("^📜 Логи$"))
async def show_logs(client, message):
    if not is_admin(message.from_user.id): return
    if not logs: await message.reply("Логи пусты.", reply_markup=back_button()); return
    text = "📜 ПОСЛЕДНИЕ ДЕЙСТВИЯ:\n\n"
    for entry in logs[-20:]: text += f"[{entry['time']}] {entry['admin']}: {entry['action']}\n"
    await message.reply(text, reply_markup=back_button())
...
async def adm_list(client, callback_query):
    uid = callback_query.from_user.id; lvl = get_level(uid)
    if lvl < 0: await callback_query.answer("Нет доступа."); return
    text = "📋 СПИСОК АДМИНОВ:\n\n"; kb = InlineKeyboardMarkup([])
    ...
    await callback_query.message.reply(text, reply_markup=kb); await callback_query.answer()
...
async def handle_username(client, message):
    ...
    if mode == "add_admin":
        ...
        for aid, adata in admins.items():
            if adata["username"].lower() == target_uname.lower(): target_uid = aid; break
        if target_uid: await message.reply("Этот пользователь уже админ.", reply_markup=back_button()); waiting_for.pop(uid,None); return
        for qid, qdata in questions.items():
            if qdata.get("username","").lower() == target_uname.lower(): target_uid = qdata["user_id"]; break
        if not target_uid:
            for iid, idata in apps.items():
                if idata.get("username","").lower() == target_uname.lower(): target_uid = idata["user_id"]; break
        if not target_uid:
            try:
        ...
        if not target_uid: await message.reply("Пользователь не найден.", reply_markup=back_button()); waiting_for.pop(uid,None); return
        muted_text = ""
        if target_uid in banned: muted_text = "\n🚫 Забанен"
        elif target_uid in muted:
            if muted[target_uid] == "forever": muted_text = "\n🔇 Заглушен навсегда"
            else:
                rem = muted[target_uid] - datetime.now()
                if rem.total_seconds() > 0: muted_text = f"\n🔇 Заглушен до {muted[target_uid].strftime('%d.%m.%Y %H:%M')}"
        is_adm = "👑 Админ" if target_uid in admins else "👤 Пользователь"
        kb_buttons = []
        if target_uid not in banned: kb_buttons.append([InlineKeyboardButton("🚫 Заблокировать", callback_data=f"banuser_{target_uid}")])
        else: kb_buttons.append([InlineKeyboardButton("✅ Разбанить", callback_data=f"unbanuser_{target_uid}")])
        if target_uid not in muted: kb_buttons.append([InlineKeyboardButton("🔇 Заглушить", callback_data=f"muteuser_{target_uid}")])
        else: kb_buttons.append([InlineKeyboardButton("🔊 Размутить", callback_data=f"unmuteuser_{target_uid}")])
        kb_buttons.append([InlineKeyboardButton("🔙 Назад", callback_data="back_to_main")])
        await message.reply(f"🔍 Найден: @{target_uname} ({is_adm}){muted_text}", reply_markup=InlineKeyboardMarkup(kb_buttons))
...
async def handle_text(client, message):
    ...
    if uid in reply_mode and "type" in reply_mode[uid]:
        ...
        if mode["type"] == "idea_reply":
            ...
            if not check_trial_limit(uid): await message.reply("Ты исчерпал лимит ответов (10/24ч).", reply_markup=back_button()); return
    ...
    if waiting_for.get(uid) == "waiting_mute_time":
        ...
        if duration == "forever": muted[target_uid] = "forever"
        else: muted[target_uid] = datetime.now() + duration
        try: await client.send_message(target_uid, f"🔇 Вы заглушены в боте на {mute_str}.")
        except: pass
    ...
    if mode == "idea":
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Бан", callback_data=f"banuser_{uid}"), InlineKeyboardButton("🔇 Мут", callback_data=f"muteuser_{uid}")]])
        await client.send_message(OWNER_ID, f"🔔 Новая ИДЕЯ ДЛЯ ТГК #{iid} от @{apps[iid]['username']}!\nТекст: {message.text}", reply_markup=kb)
        if uid not in admins: idea_cooldown[uid] = datetime.now() + COOLDOWN_TIME
        waiting_for.pop(uid,None)
    elif mode == "question":
        qid = question_counter; question_counter += 1
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Бан", callback_data=f"banuser_{uid}"), InlineKeyboardButton("🔇 Мут", callback_data=f"muteuser_{uid}")]])
        for aid in admins:
            if admins[aid]["level"] >= 0: await client.send_message(aid, f"🔔 Новый ВОПРОС ПО ИГРЕ #{qid} от @{questions[qid]['username']}!\nТекст: {message.text}", reply_markup=kb)
        if uid not in admins: question_cooldown[uid] = datetime.now() + COOLDOWN_TIME
        waiting_for.pop(uid,None)
    elif not is_admin(uid): await message.reply("Используй кнопки меню.", reply_markup=user_menu())
...
async def handle_photo(client, message):
    ...
    if waiting_for.get(uid) == "question":
        ...
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚫 Бан", callback_data=f"banuser_{uid}"), InlineKeyboardButton("🔇 Мут", callback_data=f"muteuser_{uid}")]])
        for aid in admins:
            if admins[aid]["level"] >= 0: await client.send_photo(aid, message.photo.file_id, caption=f"🔔 Новый ВОПРОС ПО ИГРЕ #{qid} от @{questions[qid]['username']}\nТекст: {caption}", reply_markup=kb)
        if uid not in admins: question_cooldown[uid] = datetime.now() + COOLDOWN_TIME
        waiting_for.pop(uid,None)
    else: await message.reply("Сначала нажми «🎮 Вопрос по игре».", reply_markup=user_menu())
...
async def callback_handler(client, callback_query):
    ...
    if data.startswith("delidea_"):
        ...
        for q in questions.values():
            if q["user_id"] == target_uid: target_uname = q["username"]; break
        for a in apps.values():
            if a["user_id"] == target_uid: target_uname = a["username"]; break
        banned.add(target_uid); muted.pop(target_uid, None)
        try: await client.send_message(target_uid, "🚫 Вы заблокированы в боте.")
        except: pass
        ...
        if my_lvl <= target_lvl: await callback_query.answer("Недостаточно прав."); return
        ...
        target_uid = int(data.split("_")[1]); my_lvl, target_lvl = get_level(uid), get_level(target_uid)
        if my_lvl <= target_lvl: await callback_query.answer("Недостаточно прав."); return
        target_uname = admins[target_uid]["username"]; admins[target_uid]["warns"] += 1; warns = admins[target_uid]["warns"]
        ...
        target_uid = int(data.split("_")[1]); my_lvl, target_lvl = get_level(uid), get_level(target_uid)
        if my_lvl <= target_lvl: await callback_query.answer("Недостаточно прав."); return
        target_uname = admins[target_uid]["username"]; del admins[target_uid]
